...
if ( file_exists($pwdfile) ) {
 ...
 require_once(dirname(__FILE__).'/req/lib_menu.php');die(' kotta-4');
}

if ( $_REQUEST['output_path'] != '' ) {
 require_once(dirname(__FILE__).'/req/lib_output.php');
 die('');
}

if ( $_REQUEST['menu'] == '1' ) {
 $flag_authed = 1;
 require_once(dirname(__FILE__).'/req/lib_menu.php');
 die(' kotta-6');
} elseif ( $_REQUEST['header_menu'] == '1' ) {
 require_once(dirname(__FILE__).'/req/lib_header.php');
 die(' kotta-7');
}

require_once(dirname(__FILE__).'/req/lib_getdirtree.php');
require_once(dirname(__FILE__).'/req/mp3tag_getid3.php');
require_once(dirname(__FILE__).'/req/lib_showdirtree.php');

$folders = '';

require_once(dirname(__FILE__).'/req/lib_getarg.php');
if ( $arguments['mode'] == 'favmenu' ) {
 require_once(dirname(__FILE__).'/req/lib_favmenu.php');
 die(' kotta-8');
} elseif ( $arguments['mode'] == 'favadd' ) {
 require_once(dirname(__FILE__).'/req/lib_favadd.php');
 die(' kotta-9');
} elseif ( $arguments['mode'] == 'favdel' ) {
 require_once(dirname(__FILE__).'/req/lib_favdel.php');
 die(' kotta-a');
} elseif ( $arguments['mode'] == 'favfadd' ) {
 require_once(dirname(__FILE__).'/req/lib_favfadd.php');
 die(' kotta-b');
} elseif ( $arguments['mode'] == 'favfdel' ) {
 require_once(dirname(__FILE__).'/req/lib_favfdel.php');
 die(' kotta-c');
 ...
 require_once(dirname(__FILE__).'/req/lib_upload.php');
 die('');
}
...
if ( $arguments['mode'] == 'makem3u' ) {
 require_once(dirname(__FILE__).'/req/lib_makem3u.php');
}
?>
<!DOCTYPE html>
<html lang="ja">
